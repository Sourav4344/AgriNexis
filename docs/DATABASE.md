...
| `produce_listings` | `id`, `farmer_profile_id`, crop/variety, quantity/available quantity, unit, harvest/availability dates, location, quality summary, status, version |
...
| `recommendation_runs` | `id`, listing, input/version metadata, calculated/valid times, mode/confidence; reproducibility metadata, not an accepted transaction |
| `recommendation_options` | run/opportunity reference, rank, gross and itemized estimated costs, estimated NFR, trust/confidence, explanation facts |
